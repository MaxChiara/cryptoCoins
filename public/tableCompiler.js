...
function compilaHomeTable(data, tableKeys, tableValues, numRows=50) { // tableKeys = list headers tabella - !!hardcoded in rowCrerator!!
		for (i=0; i<tableKeys.length; i++) { //colonne						 tableValues = list dati corrispondenti a tableKeys, 
		let classArray = document.getElementsByClassName(tableKeys[i]);//percorso nel file json: data.data[i].tableValues[i]	
		for (ii=0; ii<numRows; ii++) {	 //righe
...
function compilaQuoteTable (data, tableKeys, tableValues, numRows, ids) { // come sopra ma usa 'id' per accedere ai dati e non index (loop oggetto, non lista)
	for (i=0; i<tableKeys.length; i++) { //colonne						 
		let classArray = document.getElementsByClassName(tableKeys[i]);//percorso nel file json: data.data[i].tableValues[i]	
		for (ii=0; ii<numRows; ii++) {	 //righe
...
function formatValues() {	//formatta i valori di valueFormat
	for (i=0; i<valueFormat.length; i++) {
		...
		for (j=0; j<classArray.length; j++){
			...
			classArray[j].textContent = new Intl.NumberFormat('de-DE', {style: 'currency', currency: 'EUR'}).format(val);
		}
	}
	colorValues()
}
...
function colorValues () {	//funzione che cambia solo il colore di 'Value'
	for (i=0; i<valueFormat.length; i++) {
		let classArray = document.getElementsByClassName(valueFormat[i]);
		for (j=0; j<classArray.length; j++){
...
function formatNumbers() {	//formatta i valori di numberFormat
	for (i=0; i<numberFormat.length; i++) {
		...
		for (j=0; j<classArray.length; j++){
			...
			if (classArray[j].textContent == '0') {classArray[j].textContent = '/'}
		}
}
}

function formatGreenRed() {	//formatta i valori di greenRedFormat
...
function clearTable(remaining = 0) { //remaining = quanti td si vuole lasciare, default nessuno.
...
function createIdList(data) {
	data.data.forEach(element => {idList.push(element['id'])
	});
}

function scrollRowsA() {	//funzione che scorre tabella in avanti
	currentIdTabella += homeNumRows;
	if (currentIdTabella>=idList.length) {currentIdTabella = 0}
...
function scrollRowsB() {	//funzione che scorre tabella indietro
	currentIdTabella -= homeNumRows;
	if (currentIdTabella<0) {currentIdTabella = (idList.length - homeNumRows)}
	start = currentIdTabella;
	let ids = idList.slice(currentIdTabella, currentIdTabella+homeNumRows);
	scorriHomeTable(ids);
}
...
function scrollArticlesA() {				//funzione che scorre gli articles in avanti
	currentId += homeNumRows;				// aumento currentId del numero di valute mostrate
	if (currentId>=idList.length) {currentId = 0}	//controllo se dobbiamo scorrere da capo
	let ids = idList.slice(currentId, currentId+homeNumRows)
	cryptoInfoWrapper(ids)
}

function scrollArticlesB() {				//funzione che scorre gli articles idietro
	currentId -= homeNumRows;				// aumento currentId del numero di valute mostrate
	if (currentId<0) {currentId = (idList.length - homeNumRows)}	//controllo se dobbiamo tornare alle fine
	let ids = idList.slice(currentId, currentId+homeNumRows)
	cryptoInfoWrapper(ids)
}

function scrollA() {
	if (scrollTabella) {scrollRowsA()}
	if (scrollArticles) {scrollArticlesA()}
}

function scrollB() {
	if (scrollTabella) {scrollRowsB()}
	if (scrollArticles) {scrollArticlesB()}
}
...
//crypto info request
function cryptoInfoWrapper(ids) { //ids = id delle valute che deve fecthare
